const sk8 = sk => {
  ...
  sk.setup = () => {
    ...
    posX = sk.displayWidth * 0.5 - largura * 0.5
    posY = sk.displayHeight * 0.5 - altura * 0.5
    sk.frameRate(30)
  }
  sk.draw = () => {
    ...
    posXloop = sk.random(-0.5 * largura, sk.width - largura * 0.5)
    posYloop = sk.random(-0.5 * altura, sk.height - altura * 0.5)
    for (let x = 0; x < 10; x++) {
      sk.rect(posXloop, posYloop, largura + mouseXtemp, altura + mouseYtemp)
    }
    if (
      sk.mouseX > posX &&
